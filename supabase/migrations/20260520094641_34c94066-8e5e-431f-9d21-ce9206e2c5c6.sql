-- Trusted variant: accepts a pre-computed similarity (0..100). Only service_role may call.
CREATE OR REPLACE FUNCTION public.submit_daily_attempt_scored(
  p_user_id uuid,
  p_guess text,
...
REVOKE EXECUTE ON FUNCTION public.submit_daily_attempt_scored(uuid, text, integer, integer) FROM PUBLIC, anon, authenticated;
