...
- [x] Regra de tempos p/ salas grandes (2026-07-27, pedido do Tito): até 6 jogadores os tempos originais intocados; de 7 a 12, TODOS os tempos de fase escalam por n/6 (12 = 2x) p/ dar tempo de ler todas as cédulas. Fonte autoritativa `public.phase_secs` (migration 20260727120000) aplicada em escolha/escrita/votação/prorrogações/próxima rodada + espera da revelação (com +6s de folga p/ coreografia); espelho client `src/lib/game-times.ts` nos holds da revelação (12s) e placar (8s), barras de tempo e otimismo do chooseWord. Verificado: 6 unit novos, `scripts/test-phase-secs.mjs` 10/10 no banco (4→12 jogadores, expulsos não contam), test-e2e-round 14/14, Playwright 2/2. Deploy `cd42184`
- [x] Fase 6 · lado do repo (2026-07-27): ícones adaptativos + monocromático e splash gerados da marca real (`scripts/gen-brand-assets.mjs` → `resources/` → capacitor-assets); versionCode 2 / versionName 2.0.0; R8 ligado com keep-rules Capacitor; testes no pacote `app.verbete.game`; colors.xml da marca; deep links `/?join=` (autoVerify + esquema próprio) com `appUrlOpen` roteando no SPA; botão voltar (`src/lib/native.ts`); assinatura via `key.properties` fora do repo; workflow `android.yml` (APK debug a cada push — **primeiro build verde no CI**, AAB assinado via dispatch quando houver secrets); `docs/store-checklist.md` completo. Deploy web `6cb8bd2`. **Pendências do Tito: keystore + 4 secrets no GitHub, conta Play Console, OAuth Google (Client ID/Secret → eu ativo no Supabase e reativo os botões), SHA-256 p/ assetlinks.json**
- [x] Fase 7 (a11y & perf, 2026-07-27): **0 violações WCAG 2.1 AA** nas 5 telas públicas (auditoria axe-core reproduzível em `scripts/audit-a11y.mjs`; falso positivo do wordmark documentado — axe lê o contorno text-stroke como cor do texto). Correções: zoom liberado até 5x (maximum-scale=1 violava 1.4.4), `MotionConfig reducedMotion="user"` (Framer respeita o SO), timer com `role=timer` + anúncios só em marcos (10s/3s) em vez de cada segundo, cortina de fase com anúncio sr-only, mascote decorativo. Perf: vendors estáveis (react 60KB + supabase 55KB + motion 42KB gz) em chunks próprios só no client — bundle único de 228KB gz virou app de 75KB; a cada deploy o jogador rebaixa só o app (antes: tudo). Validado com E2E Playwright completo CONTRA a produção. Deploy `036fdb3`
- [x] Fase 8 (observabilidade própria, 2026-07-27): tabela `ops_events` (RLS total; ingestão via `log_ops_event` com whitelist + rate-limit 30/5min por sessão; retenção 30d no cron) recebendo: erros de client (window.onerror/unhandledrejection), crashes de React (ErrorBoundary com visual Verbete no lugar da tela branca), falhas de RPC 5xx/rede (fetch do supabase instrumentado — zero call sites tocados), quedas de canal realtime e avanços do backstop. Privacidade: só hashes não reversíveis. Painel `/admin/ops` (role admin): funil (salas→partidas→rodadas→concluídas + diário) e saúde em janelas 24h/3d/7d. Telemetria via fetch puro (funciona mesmo com o SDK quebrado) com throttle 10/min + dedupe. Verificado: `scripts/test-ops.mjs` 6/6 + prova fim-a-fim em produção (erro sintético na página real → gravado com build e sessão). Deploy `eca28d1`
- [x] Fase 10 · parte 1 (qualidade de tipos, 2026-07-27): workflow `gen-types.yml` regenera `types.ts` das migrations no CI e commita de volta (fim do drift); **43 casts `as any`/`as never` de RPC eliminados** — toda chamada agora é validada pelo compilador contra o schema real; `noUncheckedIndexedAccess` ligado (36 acessos por índice corrigidos); código morto removido (RevealPhase/PhaseShared órfãos); lint 107 → 67 avisos. Validado: typecheck limpo, 36 unit, 2/2 E2E. Deploy `b502ee8`
- [x] Fase 10 · parte 2 (split do use-room, 2026-07-27): **873 → 124 linhas** — o hook virou orquestrador puro (estado + `RoomCtx`) compondo 5 módulos em `hooks/room/`: `use-room-data` (carga inicial/rodada/palavra), `use-room-realtime` (canal: postgres_changes/broadcast/presença/telemetria), `use-room-optimistic` (CustomEvents), `use-room-polling` (poll de fase, poll de assinatura 2,5s, adaptativo 25s, offline/online, retomada mobile), `use-room-presence` (merge memoizado). Movimentação pura em 4 commits, cada um com o E2E multiplayer verde; validação final com a suíte completa E CONTRA a produção. Lint 107 → 63 avisos na fase toda. **FASE 10 CONCLUÍDA** (restam 63 avisos de exhaustive-deps/react-refresh — débito cosmético documentado). Deploy `1bffef6`
- [x] Fase 6 · finalização automatizável (2026-07-28): keystore RSA-4096 gerado FORA do repo (`C:\Users\titov\verbete-release\` — **Tito: fazer backup!**); 4 secrets cadastrados no GitHub via API (sealed box libsodium); `/.well-known/assetlinks.json` publicado com o SHA-256 real (App Links verificados); **AAB ASSINADO gerado no CI** (artifact `verbete-release-aab`, 4,2MB) via workflow_dispatch; ficha da loja pronta em `resources/store/` (ícone 512, feature graphic com slogan, 7 screenshots de partida REAL em produção via `gen-store-shots.mjs`). Restam só ações com login do Tito: conta Play Console (US$25) + upload do AAB; OAuth Google (Client ID/Secret → eu ativo e reativo botões); se ligar Play App Signing, adicionar o SHA-256 do Google ao assetlinks
- [x] Fase 9 · lotes 2-4 (2026-07-28): três passadas do pipeline (gera + verifica em dupla IA) → **533 → 1.078 publicadas** (+545; 182 reprovadas pelo verificador; varredura de circulares limpa). Rendimento por passada caiu 321→150→74: os 15 temas atuais saturaram. Para as ~420 restantes até 1.500: adicionar categorias novas ao gerador — como aparecem no seletor do Lobby, a lista precisa do OK do Tito
- [~] Fase 9 · rumo a 1.500: **ADIADA por decisão do Tito (2026-07-29)** — jogo segue com 1.078 publicadas; retomar com categorias novas quando ele quiser

- [x] Auditoria externa aplicada (2026-07-29): ① **achado crítico** — `apply_similarity_bonus` não-idempotente + edge pública `score-similarity` aceitava texto do chamador → replay/forja de pontos. Corrigido: assinatura `(room_id, round, ids)` atômica (só premia quem virou near_truth agora), edge ignora texto do chamador e lê o real de `definitions` restrito à sala/rodada, assinatura antiga vira no-op. Verificado: `scripts/test-similarity-bonus.mjs` 6/6 + prova E2E da edge em produção (texto forjado ignorado, replay não soma). ② `npm audit` 6→0 vulnerabilidades. ③ `.gitattributes` eol=lf + `.wrangler/`/`supabase/.temp/` removidos do git. ④ `inputValidator`→`validator`. CI verde nos 3 workflows; deploy edge + web `86c6900`, tipos `e4298ce`.
