...
import {
  fetchDailyChallenge, fetchTodayAttempt, fetchDailyLeaderboard,
  fetchAllAchievements, msUntilNextChallenge, formatHMS,
  type DailyChallenge, type DailyReview,
} from "@/lib/daily";
...
function DailyPage() {
  ...
  const [board, setBoard] = useState<Awaited<ReturnType<typeof fetchDailyLeaderboard>>>([]);
  const [achMap, setAchMap] = useState<Map<string, Awaited<ReturnType<typeof fetchAllAchievements>>[number]>>(new Map());
  const [guess, setGuess] = useState("");
  const [busy, setBusy] = useState(false);
  const [result, setResult] = useState<{ correct: boolean; score: number; truth: string; similarity: number; guess: string; timeSeconds: number; streak?: number } | null>(null);
  ...
  const remaining = Math.max(0, DAILY_TIME_LIMIT - Math.floor((tickNow - startedAt) / 1000));
  ...
  const onSubmit = async () => {
    ...
    setBusy(true);
    try {
      const timeSeconds = Math.min(600, Math.round((Date.now() - startedAt) / 1000));
      const submittedGuess = guess.trim();
      const res = await submit({ data: { guess: submittedGuess, timeSeconds } });
      if (res.already_played) {
        ...
        const [lb, fresh] = await Promise.all([fetchDailyLeaderboard(), fetchTodayAttempt(user!.id)]);
        setBoard(lb);
        setReview(fresh);
      }
    } catch (e) {
    ...
    <div className="mobile-shell pt-4 gap-4 overflow-y-auto pb-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex flex-col items-end gap-0.5 rounded-2xl px-3 py-1.5 bg-card shadow-pop border border-pink/30">
          <span className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground font-display leading-none">⏳ Próximo desafio</span>
          <span
      ...
      <div className="text-center no-copy" onCopy={(e) => e.preventDefault()} onContextMenu={(e) => e.preventDefault()}>
        <p className="text-xs uppercase tracking-[0.3em] text-muted-foreground font-display">Desafio Diário</p>
        <motion.h1
        ...
        {data.word.category && (
          <p className="text-xs text-muted-foreground mt-1">{data.word.category}</p>
        )}
      </div>

      <AnimatePresence mode="wait">
        {!alreadyDone ? (
          <motion.div key="form" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }}
            className="flex flex-col gap-3">
            <TimerBar remaining={remaining} max={DAILY_TIME_LIMIT} tickStartAt={10} />
            ...
            />
            <button onClick={onSubmit} disabled={busy || !guess.trim()}
          ...
          <motion.div key="result" initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            className={"sticker py-5 px-4 text-center " + (displayResult?.correct ? "bg-mint/20 border-mint" : "")}>
            <p className="text-4xl mb-1">{displayResult?.correct ? "🎉" : "🤔"}</p>
            <p className="font-display text-xl">
              {displayResult?.correct ? "Acertou!" : "Não foi dessa vez"}
            </p>
            <p className="font-display text-3xl text-primary mt-1">+{displayResult?.score ?? 0} pts</p>

            {displayResult && (
              <div className="mt-3 text-[11px] text-muted-foreground bg-white/50 rounded-2xl px-3 py-2 border border-white/40 text-left">
                <p className="font-display uppercase tracking-wider text-[10px] mb-1 text-center">Como calculamos</p>
                {displayResult.correct ? (
                  <>
                    <p>
                      base = máx(100 − <b>{displayResult.timeSeconds}s</b>, 20) = <b>{Math.max(100 - displayResult.timeSeconds, 20)}</b>
                    </p>
                    <p>
                      pts = arred(base × <b>{displayResult.similarity}%</b>) mín. 20 = <b>{displayResult.score}</b>
                    </p>
                  </>
                ) : (
                  <>
                    <p>
                      Sua semelhança foi <b>{displayResult.similarity}%</b>, abaixo do mínimo de <b>80%</b> para pontuar.
                    </p>
                    <p className="mt-1 opacity-80">
                      Se tivesse atingido 80%+: base = máx(100 − <b>{displayResult.timeSeconds}s</b>, 20) = <b>{Math.max(100 - displayResult.timeSeconds, 20)}</b>, depois multiplicada pela semelhança (mín. 20 pts).
                    </p>
                  </>
                )}
              </div>
            ...
            {typeof displayResult?.similarity === "number" && (
              <div className="mt-4">
                <div className="flex items-center justify-between text-[11px] uppercase tracking-wider text-muted-foreground font-display mb-1">
                  ...
                  <span className={displayResult.correct ? "text-mint" : "text-pink"}>
                    {displayResult.similarity}% {displayResult.correct ? "✓" : "(min. 80%)"}
                  </span>
                </div>
                <div className="h-2 rounded-full bg-white/40 overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${Math.min(100, displayResult.similarity)}%` }}
                    transition={{ duration: 0.8, ease: "easeOut" }}
                    className={"h-full " + (displayResult.correct ? "bg-mint" : "bg-gradient-fun")}
                  />
                </div>
              </div>
            )}
            ...
            {displayResult?.guess && (
              <div className="mt-4 text-left bg-white/60 rounded-2xl p-3 border border-white/40">
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-display mb-1">Seu palpite</p>
                <p className="text-sm">"{displayResult.guess}"</p>
              </div>
            )}
            {displayResult?.truth && (
              <div className="mt-2 text-left bg-mint/15 rounded-2xl p-3 border border-mint/40">
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-display mb-1">Significado correto</p>
                <p className="text-sm italic">"{displayResult.truth}"</p>
              </div>
            )}

            {result?.streak && (
              <p className="font-display text-sm text-sun mt-3">🔥 Streak: {result.streak} dia{result.streak > 1 ? "s" : ""}</p>
            )}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div>
        ...
        {board.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-3">Seja o primeiro a jogar hoje!</p>
        ) : (
          <ul className="space-y-1.5">
            {board.map((row, i) => (
              <li key={row.user_id} className="sticker py-2 px-3 flex items-center gap-2 text-sm">
                <span className="font-display w-6 text-center text-sun">{i + 1}</span>
                <span className="text-xl">{row.profile?.avatar ?? "🦊"}</span>
                <span className="flex-1 truncate">{row.profile?.display_name ?? "Jogador"}</span>
                {row.is_correct ? <span className="text-mint text-xs">✓</span> : <span className="text-muted-foreground text-xs">✗</span>}
