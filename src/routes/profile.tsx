  ...
  head: () => ({
    meta: [
      { title: "Meu perfil — Verbete" },
      {
        name: "description",
        content:
          "Acompanhe suas estatísticas, conquistas e histórico de partidas no Verbete.",
      },
...
function ProfilePage() {
  ...
  const updateAppearance = async (
    next: Partial<Pick<Profile, "avatar" | "color">>,
  ) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ data: p }, { data: s }, { data: h }, ach, unl, { data: role }] =
        await Promise.all([
          supabase
            .from("profiles")
    ...
    <div className="mobile-shell pt-4 gap-3 overflow-y-auto pb-8">
      ...
      <div className="text-center">
        <button
          ...
          style={{
            background: `radial-gradient(circle at 30% 25%, ${profile?.color ?? "#FFD166"}cc, ${profile?.color ?? "#FFD166"})`,
          }}
        >
          {profile?.avatar ?? "🦊"}
          <span className="absolute -bottom-1 -right-1 bg-card border border-white/10 rounded-full w-7 h-7 flex items-center justify-center text-sm shadow-soft">
            ✏️
          </span>
        </button>
        {editingAvatar && (
        ...
        {editing ? (
          ...
          <h1
            ...
            className="font-display text-2xl mt-2 cursor-pointer"
          >
        ...
        return (
          <div className="sticker bg-gradient-to-r from-grape/25 to-pink/20 py-3 px-4 space-y-1.5">
            ...
            <div className="h-2 rounded-full bg-black/25 overflow-hidden border border-white/10">
              <div
                className="h-full rounded-full bg-gradient-fun transition-all"
                style={{ width: `${pct}%` }}
              />
            </div>
          </div>
      ...
      <div className="sticker bg-gradient-to-r from-sun/20 to-pink/20 py-3 px-4 flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="font-display text-xl leading-none">
            {stats?.current_streak ?? 0} dia
            {(stats?.current_streak ?? 0) !== 1 ? "s" : ""}
          </div>
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground mt-1">
            Streak atual · recorde {stats?.best_streak ?? 0}
          </div>
        </div>
        <Link
          to="/daily"
          search={{ from: "profile" }}
          className="btn-pop bg-gradient-fun text-white text-xs py-2 px-3"
        >
          Desafio diário
        </Link>
      </div>
      ...
      <div className="flex gap-2">
        <Link
          to="/ranking"
          search={{ from: "profile" }}
          className="btn-pop bg-card flex-1 text-center text-sm border border-white/10"
        >
          🏆 Ranking
        </Link>
      </div>
        ...
        {achievements.map((a) => {
          ...
          return (
            <div
              ...
              className={
                "sticker py-2 text-center transition " +
                (got ? "" : "opacity-30 grayscale")
              }
            >
              <div className="text-2xl">{a.emoji}</div>
              <div className="text-[9px] font-display leading-tight mt-1 truncate">
                {a.name}
              </div>
            </div>
          );
      ...
      {history.length === 0 ? (
        <p className="text-sm text-muted-foreground text-center py-4">
          Nenhuma partida registrada ainda. Jogue logado para aparecer aqui!
        </p>
      ) : (
          ...
          {history.map((m) => (
            ...
            >
              ...
              <span className="flex-1">
                Sala {m.room_code} · {m.players_count} jog.
              </span>
        ...
        <div className="sticker bg-destructive/5 border border-destructive/30 p-3 flex items-center justify-between gap-3">
          <div className="flex-1">
            ...
            <div className="text-[11px] text-muted-foreground leading-snug mt-0.5">
              Remove perfil, estatísticas, conquistas e e-mail. Esta ação é
              irreversível.
            </div>
          </div>
          <button
          ...
          >
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
                Vamos apagar permanentemente seu perfil, e-mail, estatísticas,
                conquistas e histórico. Você não poderá desfazer.
              </p>
            </div>
            <label className="text-xs text-muted-foreground mt-2">
              Para confirmar, digite seu apelido:{" "}
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button
                ...
                disabled={
                  deleting ||
                  deleteConfirm.trim() !== (profile?.display_name.trim() ?? "")
                }
        ...
        >
          ...
          >
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
                Vamos resetar suas partidas, vitórias, pontos, streak,
                tentativas do desafio e conquistas desbloqueadas. Sua conta e
                perfil permanecem.
              </p>
            </div>
            {resetError && (
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    setResetError(
                      e instanceof Error ? e.message : "Erro ao zerar.",
                    );
                  } finally {
                    setResetting(false);
                  }
