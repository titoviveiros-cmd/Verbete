...
let fallbackEls: Partial<Record<FallbackKind, HTMLAudioElement>> = {};
...
const FALLBACK_NOTES: Record<FallbackKind, ToneNote[]> = {
  ...
  tick: [
    { freq: 1046.5, start: 0, dur: 0.08, gain: 0.5 },
  ],
};
...
function toneDataUri(notes: ToneNote[]) {
  ...
  for (let i = 0; i < sampleCount; i++) {
    ...
    for (const n of notes) {
      ...
      mixed += Math.sin(2 * Math.PI * n.freq * local) * n.gain * Math.max(0, fade);
    }
...
async function unlockFallbackAudio() {
  ...
  const els = [getFallback("alert"), getFallback("join")].filter((el): el is HTMLAudioElement => !!el);
...
function makeImpulse(c: AudioContext, duration = 1.4, decay = 2.6): AudioBuffer {
...
function getMaster(c: AudioContext): GainNode {
  ...
  try {
    ...
    m.connect(hp).connect(shaper).connect(comp).connect(postGain).connect(c.destination);
...
function createSubtlePanner(c: AudioContext, bias = 0): StereoPannerNode | null {
  try {
    const p = c.createStereoPanner();
    p.pan.value = Math.max(-1, Math.min(1, bias + (Math.random() * 2 - 1) * 0.35));
    return p;
  } catch { return null; }
}
...
let lastDuckAt = 0;
export function registerMusicDuck(fn: DuckFn | null) { musicDuckCallback = fn; }
...
function duckMusic(_c: AudioContext, depth = 0.35, holdMs = 200) {
  ...
  lastDuckAt = now;
  try { musicDuckCallback(depth, holdMs); } catch {}
}

function scheduleTones(c: AudioContext, bus: Bus, notes: Tone[], opts?: { jitterCents?: number; duck?: boolean }) {
  ...
  for (const n of notes) {
    ...
    if (n.glideTo) {
      osc.frequency.exponentialRampToValueAtTime(Math.max(1, n.glideTo), now + n.start + n.dur);
    }
...
async function play(bus: Bus, notes: Tone[], opts?: { vibrate?: number | number[]; jitterCents?: number; duck?: boolean; fallback?: FallbackKind }) {
  if (prefs.muted) return;
  if (opts?.vibrate) vibrate(opts.vibrate);
  const c = await ensureRunning();
  if (c) { scheduleTones(c, bus, notes, opts); return; }
...
export async function playJoin() {
  await play("sfx", [
    { freq: 783.99, start: 0.00, dur: 0.16, gain: 0.20, type: "sine" },     // G5
    { freq: 1174.66, start: 0.08, dur: 0.20, gain: 0.20, type: "sine" },    // D6
    { freq: 1567.98, start: 0.18, dur: 0.36, gain: 0.18, type: "triangle" },// G6
    { freq: 3135.96, start: 0.20, dur: 0.18, gain: 0.06, type: "sine" },    // brilho harmônico
...
export async function playBotJoin() {
  await play("sfx", [
    { freq: 330, start: 0.00, dur: 0.09, gain: 0.20, type: "square", glideTo: 520 },
    { freq: 520, start: 0.11, dur: 0.09, gain: 0.20, type: "square", glideTo: 660 },
...
export async function playKick() {
  await play("sfx", [
    { freq: 520, start: 0.00, dur: 0.08, gain: 0.22, type: "sawtooth", glideTo: 180 },
    { freq: 160, start: 0.10, dur: 0.22, gain: 0.20, type: "sine", glideTo: 90 },
...
export async function playAlert() {
  await play("sfx", [
    { freq: 1174.66, start: 0.00, dur: 0.16, gain: 0.24, type: "sine" },
    { freq: 1567.98, start: 0.14, dur: 0.30, gain: 0.24, type: "sine" },
    { freq: 2349.32, start: 0.14, dur: 0.20, gain: 0.08, type: "triangle" }, // harmônico
...
export async function playSubmit() {
  await play("ui", [
    { freq: 740, start: 0.00, dur: 0.06, gain: 0.20, type: "triangle" },  // F#5
    { freq: 880, start: 0.05, dur: 0.10, gain: 0.18, type: "sine" },      // A5
...
export async function playUITap(variant: UITapVariant = "default") {
  const presets: Record<UITapVariant, { f1: number; f2: number; g1: number; g2: number; dur: number }> = {
    default:   { f1: 1760, f2: 2637, g1: 0.12, g2: 0.06, dur: 0.03 },
    primary:   { f1: 1320, f2: 1980, g1: 0.16, g2: 0.08, dur: 0.05 },
    secondary: { f1: 2349, f2: 3520, g1: 0.10, g2: 0.05, dur: 0.025 },
    soft:      { f1: 1568, f2: 2349, g1: 0.06, g2: 0.03, dur: 0.025 },
  };
  const p = presets[variant];
  await play("ui", [
    { freq: p.f1, start: 0.00, dur: p.dur,       gain: p.g1, type: "triangle" },
...
export async function playVoteCast() {
  ...
  const osc = c.createOscillator();
  osc.type = "sawtooth";
  osc.frequency.setValueAtTime(196.00, now); // G3 (corda solta)
  ...
  g.gain.exponentialRampToValueAtTime(0.30 * gM, now + 0.005); // ataque 5ms
...
export async function playVoteReceived() {
  await play("sfx", [
    { freq: 1396.91, start: 0.00, dur: 0.20, gain: 0.22, type: "sine" },  // F6
    { freq: 2093.00, start: 0.08, dur: 0.28, gain: 0.18, type: "sine" },  // C7
...
export async function playPointGained() {
  await play("sfx", [
    { freq: 698.46,  start: 0.00, dur: 0.09, gain: 0.22, type: "triangle" }, // F5
    { freq: 880.00,  start: 0.07, dur: 0.09, gain: 0.22, type: "triangle" }, // A5
    { freq: 1046.50, start: 0.14, dur: 0.09, gain: 0.22, type: "triangle" }, // C6
    { freq: 1396.91, start: 0.21, dur: 0.10, gain: 0.24, type: "triangle" }, // F6
    { freq: 2093.00, start: 0.31, dur: 0.26, gain: 0.18, type: "sine" },     // C7 sustain
...
export async function playCorrectReveal() {
  await play("sfx", [
    ...
    { freq: 392.00, start: 0.00, dur: 0.18, gain: 0.18, type: "triangle", glideTo: 1046.50 },
    ...
    { freq: 2093.00, start: 0.22, dur: 0.55, gain: 0.08, type: "sine" }, // C7 brilho
...
export async function playRoundWin() {
  await play("sfx", [
    { freq: 392.00,  start: 0.00, dur: 0.13, gain: 0.26, type: "triangle" }, // G4
    ...
    { freq: 1567.98, start: 0.40, dur: 0.30, gain: 0.16, type: "sine" },     // G6 coroa
...
export async function playGameWin() {
  await play("sfx", [
    ...
    { freq: 196.00,  start: 0.00, dur: 0.28, gain: 0.18, type: "triangle" }, // G3
    { freq: 392.00,  start: 0.00, dur: 0.28, gain: 0.20, type: "triangle" }, // G4
    { freq: 493.88,  start: 0.00, dur: 0.28, gain: 0.18, type: "triangle" }, // B4
    { freq: 587.33,  start: 0.00, dur: 0.28, gain: 0.16, type: "sine" },     // D5
    // === V: Lá maior (A-C#-E) — 0.30s, 220ms — tensão dominante ===
    { freq: 220.00,  start: 0.30, dur: 0.28, gain: 0.18, type: "triangle" }, // A3
    { freq: 440.00,  start: 0.30, dur: 0.28, gain: 0.20, type: "triangle" }, // A4
    { freq: 554.37,  start: 0.30, dur: 0.28, gain: 0.18, type: "triangle" }, // C#5
    { freq: 659.25,  start: 0.30, dur: 0.28, gain: 0.16, type: "sine" },     // E5
    // === I: Ré maior add9 (D-F#-A-D-E) — 0.62s, sustain 0.95s — resolução ===
    { freq: 146.83,  start: 0.62, dur: 1.10, gain: 0.20, type: "triangle" }, // D3 (raiz grave)
    { freq: 293.66,  start: 0.62, dur: 1.00, gain: 0.20, type: "sine" },     // D4
    { freq: 369.99,  start: 0.62, dur: 1.00, gain: 0.18, type: "sine" },     // F#4
    { freq: 440.00,  start: 0.62, dur: 1.00, gain: 0.18, type: "sine" },     // A4
    { freq: 587.33,  start: 0.62, dur: 1.00, gain: 0.16, type: "sine" },     // D5
    { freq: 659.25,  start: 0.68, dur: 0.95, gain: 0.12, type: "sine" },     // E5 (9ª)
    { freq: 880.00,  start: 0.70, dur: 0.90, gain: 0.10, type: "sine" },     // A5 brilho
...
export async function playEmojiReaction(seed = 0) {
  ...
  await play("ui", [
    { freq: base,        start: 0.00, dur: 0.05, gain: 0.18, type: "sine" },
    { freq: base * 1.5,  start: 0.03, dur: 0.09, gain: 0.16, type: "triangle" },
...
export async function playCountdownTick(remaining?: number) {
  ...
  await play("sfx", [
    { freq, start: 0, dur: 0.10, gain, type: "square" },
  ], { vibrate: 30, duck: true, fallback: "tick" });
}

/** Buzina final — trítono descendente + nota grave longa (tensão resolvida em peso). */
export async function playCountdownFinal() {
  await play("sfx", [
    { freq: 1760, start: 0.00, dur: 0.20, gain: 0.36, type: "sawtooth" }, // A6
    { freq: 1244, start: 0.20, dur: 0.20, gain: 0.36, type: "sawtooth" }, // D#6 (trítono)
    { freq: 880,  start: 0.40, dur: 0.20, gain: 0.38, type: "sawtooth" }, // A5
...
async function playNoise(opts: NoiseOpts) {
  ...
  filter.frequency.exponentialRampToValueAtTime(Math.max(20, opts.filterEnd), now + opts.duration);
  const g = c.createGain();
...
export async function playWhooshUp() {
  await playNoise({
    duration: 0.55, filterType: "bandpass", filterStart: 280, filterEnd: 4200,
    q: 1.4, peakGain: 0.16, attack: 0.06, release: 0.25, duck: true,
  });
}
...
export async function playWhooshDown() {
  await playNoise({
    duration: 0.5, filterType: "bandpass", filterStart: 3800, filterEnd: 220,
    q: 1.4, peakGain: 0.28, attack: 0.05, release: 0.22, duck: true,
  });
}
...
export async function playImpact() {
  await playNoise({
    duration: 0.45, filterType: "lowpass", filterStart: 800, filterEnd: 80,
    q: 0.9, peakGain: 0.55, attack: 0.005, release: 0.35, duck: true, vibrate: 60,
  });
}
...
export async function playCrowdAah() {
  await playNoise({
    duration: 1.1, filterType: "bandpass", filterStart: 480, filterEnd: 720,
    q: 3.5, peakGain: 0.22, attack: 0.18, release: 0.5, duck: true,
...
export async function playCrowdOoh() {
  await playNoise({
    duration: 0.85, filterType: "bandpass", filterStart: 380, filterEnd: 520,
    q: 4.5, peakGain: 0.2, attack: 0.12, release: 0.4, duck: true,
...
export async function playRevealStinger() {
  await play("sfx", [
    ...
    { freq: 110.00, start: 0.00, dur: 0.95, gain: 0.18, type: "sawtooth" }, // A2
    { freq: 130.81, start: 0.00, dur: 0.95, gain: 0.14, type: "sawtooth" }, // C3
    { freq: 164.81, start: 0.00, dur: 0.95, gain: 0.14, type: "sawtooth" }, // E3
    { freq: 196.00, start: 0.05, dur: 0.85, gain: 0.10, type: "sine" },     // G3
    { freq: 392.00, start: 0.10, dur: 0.78, gain: 0.06, type: "sine" },     // G4 brilho
  ], { duck: true });
}
...
export async function playFooled() {
  await play("sfx", [
    { freq: 466.16, start: 0.00, dur: 0.22, gain: 0.24, type: "sawtooth", glideTo: 369.99 }, // Bb4 -> F#4
    { freq: 369.99, start: 0.22, dur: 0.24, gain: 0.22, type: "sawtooth", glideTo: 293.66 }, // F#4 -> D4
    { freq: 293.66, start: 0.46, dur: 0.40, gain: 0.22, type: "triangle", glideTo: 220.00 }, // D4 -> A3
...
export async function playFooledOthers() {
  await play("sfx", [
    { freq: 587,  start: 0.00, dur: 0.05, gain: 0.18, type: "triangle" },
    { freq: 784,  start: 0.05, dur: 0.05, gain: 0.18, type: "triangle" },
    { freq: 698,  start: 0.10, dur: 0.05, gain: 0.18, type: "triangle" },
    { freq: 932,  start: 0.15, dur: 0.05, gain: 0.18, type: "triangle" },
    { freq: 1047, start: 0.20, dur: 0.06, gain: 0.20, type: "triangle" },
    { freq: 1397, start: 0.26, dur: 0.14, gain: 0.22, type: "triangle" },
    { freq: 2093, start: 0.28, dur: 0.10, gain: 0.06, type: "sine" }, // brilho
...
export async function playDrumRoll(durationMs = 900) {
  ...
  let t = 0;
  let interval = 0.10; // começa lento
  while (t < totalSec) {
    const osc = c.createOscillator();
    const g = c.createGain();
    osc.type = "sine";
...
export async function playClockTick() {
  await play("ui", [
    { freq: 2200, start: 0.00, dur: 0.02, gain: 0.06, type: "triangle" },
    { freq: 1100, start: 0.00, dur: 0.03, gain: 0.04, type: "sine" },
  ], { jitterCents: 12 });
}
...
export async function playRevealBuildUp(): Promise<void> {
  ...
  setTimeout(() => { void playRiser(420); }, 700);
  ...
  setTimeout(() => {
    void playNoise({
      ...
      q: 0.7,
      peakGain: 0.18,
      attack: 0.30, // crescendo lento (quase todo o som é attack)
  ...
  setTimeout(() => { void playCrash(); }, 1140);
}
...
export async function playCrowdReactionFooled(count: number) {
  if (count >= 3) {
    await playCrowdOoh();
    setTimeout(() => { void playCrowdAah(); }, 350);
  } else if (count >= 2) {
    await playCrowdOoh();
  } else if (count === 1) {
    // micro "ooh" — usa Aah pequeno
    await play("sfx", [
      { freq: 420, start: 0.00, dur: 0.35, gain: 0.10, type: "sine", glideTo: 360 },
      { freq: 520, start: 0.00, dur: 0.35, gain: 0.08, type: "sine", glideTo: 440 },
    ], { duck: true });
  }
}
...
export async function playPointMagnitude(points: number) {
  ...
  if (points >= 3) {
    ...
    await play("sfx", [
      { freq: 392.00,  start: 0.00, dur: 0.10, gain: 0.26, type: "triangle" }, // G4
      { freq: 493.88,  start: 0.08, dur: 0.10, gain: 0.26, type: "triangle" }, // B4
      { freq: 587.33,  start: 0.16, dur: 0.10, gain: 0.26, type: "triangle" }, // D5
      { freq: 783.99,  start: 0.24, dur: 0.18, gain: 0.30, type: "triangle" }, // G5
      // acorde G maior sustentado
      { freq: 392.00,  start: 0.40, dur: 0.55, gain: 0.18, type: "sine" },
      { freq: 493.88,  start: 0.40, dur: 0.55, gain: 0.16, type: "sine" },
      { freq: 587.33,  start: 0.40, dur: 0.55, gain: 0.16, type: "sine" },
      { freq: 783.99,  start: 0.40, dur: 0.55, gain: 0.14, type: "sine" },
      // sino agudo de coroa
      { freq: 1567.98, start: 0.45, dur: 0.50, gain: 0.10, type: "sine" },
      { freq: 2349.32, start: 0.48, dur: 0.40, gain: 0.06, type: "sine" },
  ...
  if (points === 2) {
    await play("sfx", [
      ...
      { freq: 2093.00, start: 0.28, dur: 0.30, gain: 0.18, type: "sine" },     // C7 sustain
      { freq: 2793.83, start: 0.30, dur: 0.22, gain: 0.08, type: "sine" },     // brilho
...
export async function playFooledOthersMagnitude(count: number) {
  ...
  if (count >= 3) {
    ...
    if (c) {
      ...
      const now = c.currentTime + 0.01;
      // Rimshot grave
      const k = c.createOscillator(); k.type = "square";
      ...
      k.connect(kg).connect(out); k.start(now); k.stop(now + 0.2);
    }
    // Risada subindo + fanfarra "GOTCHA"
    await play("sfx", [
      { freq: 587,  start: 0.00, dur: 0.04, gain: 0.20, type: "triangle" },
      { freq: 784,  start: 0.04, dur: 0.04, gain: 0.20, type: "triangle" },
      { freq: 988,  start: 0.08, dur: 0.04, gain: 0.20, type: "triangle" },
      { freq: 1175, start: 0.12, dur: 0.04, gain: 0.20, type: "triangle" },
      { freq: 1568, start: 0.16, dur: 0.06, gain: 0.22, type: "triangle" },
      // fanfarra "ta-DAA"
      { freq: 783.99,  start: 0.30, dur: 0.10, gain: 0.26, type: "triangle" }, // G5
      { freq: 1046.50, start: 0.40, dur: 0.10, gain: 0.26, type: "triangle" }, // C6
      { freq: 1318.51, start: 0.50, dur: 0.28, gain: 0.30, type: "triangle" }, // E6
      ...
      { freq: 1046.50, start: 0.55, dur: 0.65, gain: 0.14, type: "sine" },
      // sino top
      { freq: 2093.00, start: 0.62, dur: 0.58, gain: 0.08, type: "sine" },
    ], { vibrate: [80, 60, 80, 60, 220], duck: true });
    // "OOH" de público logo depois reforça
    setTimeout(() => { void playCrowdOoh(); }, 180);
    return;
  }
  if (count === 2) {
    ...
    await play("sfx", [
      { freq: 587,  start: 0.00, dur: 0.05, gain: 0.18, type: "triangle" },
      { freq: 784,  start: 0.05, dur: 0.05, gain: 0.18, type: "triangle" },
      { freq: 988,  start: 0.10, dur: 0.05, gain: 0.18, type: "triangle" },
      { freq: 1175, start: 0.15, dur: 0.06, gain: 0.20, type: "triangle" },
      { freq: 1568, start: 0.22, dur: 0.18, gain: 0.24, type: "triangle" },
      { freq: 2093, start: 0.28, dur: 0.20, gain: 0.10, type: "sine" },
      { freq: 3136, start: 0.30, dur: 0.16, gain: 0.05, type: "sine" }, // sparkle
...
export async function playFooledMagnitude(totalFooled: number) {
  if (totalFooled >= 3) {
    ...
    await play("sfx", [
      { freq: 466.16, start: 0.00, dur: 0.24, gain: 0.26, type: "sawtooth", glideTo: 369.99 },
      { freq: 369.99, start: 0.24, dur: 0.26, gain: 0.24, type: "sawtooth", glideTo: 277.18 },
      { freq: 277.18, start: 0.50, dur: 0.30, gain: 0.24, type: "sawtooth", glideTo: 196.00 },
      { freq: 196.00, start: 0.80, dur: 0.42, gain: 0.26, type: "triangle", glideTo: 130.81 },
      // thud final
      { freq: 110.00, start: 1.20, dur: 0.40, gain: 0.30, type: "sine", glideTo: 55.00 },
...
export async function playStreak(streakCount: number) {
  ...
  for (let i = 0; i < notes; i++) {
    ...
    osc.type = "triangle";
    osc.frequency.setValueAtTime(baseFreq * Math.pow(1.122, i * 2), now + i * 0.08);
    const peak = 0.22 * gM;
    ...
    osc.connect(g);
    if (pan) { g.connect(pan); chain = pan; }
    chain.connect(out);
...
export async function playPerfectRound() {
  ...
  setTimeout(() => {
    void play("sfx", [
      ...
      { freq: 523.25,  start: 0.00, dur: 0.12, gain: 0.30, type: "triangle" },
      { freq: 659.25,  start: 0.10, dur: 0.12, gain: 0.30, type: "triangle" },
      { freq: 783.99,  start: 0.20, dur: 0.12, gain: 0.30, type: "triangle" },
      { freq: 1046.50, start: 0.30, dur: 0.32, gain: 0.34, type: "triangle" },
      // Acorde sustentado C maj9
      { freq: 523.25,  start: 0.50, dur: 1.10, gain: 0.20, type: "sine" },
      { freq: 659.25,  start: 0.50, dur: 1.10, gain: 0.18, type: "sine" },
      { freq: 783.99,  start: 0.50, dur: 1.10, gain: 0.18, type: "sine" },
      { freq: 1046.50, start: 0.50, dur: 1.10, gain: 0.16, type: "sine" },
      { freq: 1318.51, start: 0.50, dur: 1.10, gain: 0.12, type: "sine" }, // 9ª
      // Sino dourado top
      { freq: 2093.00, start: 0.55, dur: 1.00, gain: 0.10, type: "sine" },
      { freq: 3135.96, start: 0.60, dur: 0.90, gain: 0.06, type: "sine" },
    ], { vibrate: [80, 60, 80, 60, 200, 80, 200], duck: true });
  }, 120);
  setTimeout(() => { void playCrowdOoh(); }, 350);
  setTimeout(() => { void playCrowdAah(); }, 700);
}
...
export async function playSavage() {
  ...
  setTimeout(() => {
    void play("sfx", [
      ...
      { freq: 880, start: 0.00, dur: 0.18, gain: 0.22, type: "sawtooth", glideTo: 220 },
      // Risada aguda subindo
      { freq: 587,  start: 0.20, dur: 0.05, gain: 0.20, type: "square" },
      { freq: 740,  start: 0.25, dur: 0.05, gain: 0.20, type: "square" },
      { freq: 880,  start: 0.30, dur: 0.05, gain: 0.20, type: "square" },
      { freq: 1109, start: 0.35, dur: 0.05, gain: 0.20, type: "square" },
      { freq: 1397, start: 0.40, dur: 0.08, gain: 0.22, type: "square" },
      { freq: 1760, start: 0.48, dur: 0.20, gain: 0.26, type: "triangle" },
      // Acorde dissonante grave (suspense malicioso)
      { freq: 110.00, start: 0.65, dur: 0.75, gain: 0.22, type: "sawtooth" }, // A2
      { freq: 138.59, start: 0.65, dur: 0.75, gain: 0.18, type: "sawtooth" }, // C#3
      { freq: 220.00, start: 0.70, dur: 0.70, gain: 0.16, type: "sine" },     // A3
    ], { vibrate: [60, 40, 60, 40, 60, 40, 220], jitterCents: 12, duck: true });
  }, 100);
  setTimeout(() => { void playCrowdOoh(); }, 350);
  setTimeout(() => { void playCrowdAah(); }, 1100);
}
