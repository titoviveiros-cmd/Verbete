...
import { BOT_NAMES, BOT_FAKE_DEFINITIONS_TEMPLATES, randomBotDef } from "@/lib/bot-names";
...
describe("banco de bots", () => {
  it("pool de blefes grande (playtest: 14 repetia rápido) e sem duplicatas", () => {
    ...
    expect(new Set(BOT_FAKE_DEFINITIONS_TEMPLATES).size).toBe(BOT_FAKE_DEFINITIONS_TEMPLATES.length);
  ...
  it("randomBotDef é determinístico pelo seed e cobre o pool", () => {
    ...
    expect(randomBotDef(BOT_FAKE_DEFINITIONS_TEMPLATES.length + 3)).toBe(BOT_FAKE_DEFINITIONS_TEMPLATES[3]);
  });
});
