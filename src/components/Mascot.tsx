 ...
 */
export function Mascot({ mood = "idle", size = 160, className = "" }: MascotProps) {
    ...
    >
      <svg viewBox="0 0 200 200" width={size} height={size}>
        ...
        <g transform="translate(20 30)">
          <rect x="0" y="0" width="160" height="120" rx="12"
            fill="#7B2CBF" stroke="#3A0A60" strokeWidth="4" />
          {/* pages stack */}
          <rect x="6" y="6" width="148" height="108" rx="8" fill="#FFF6E0" />
          <rect x="10" y="10" width="140" height="100" rx="6" fill="#FFFCEF" />

          {/* spine highlight */}
          <rect x="76" y="0" width="8" height="120" fill="#3A0A60" opacity="0.6" />

          {/* face */}
          <g transform="translate(80 60)">
            {/* left eye */}
            <circle cx="-22" cy="0" r="14" fill="#fff" stroke="#1A0936" strokeWidth="3" />
            <motion.circle
              cx="-22" cy="0"
              r={isWow ? 8 : 6}
              fill="#1A0936"
            ...
            {/* mouth */}
            {isSad ? (
              <path d="M -14 26 Q 0 16 14 26" stroke="#1A0936" strokeWidth="3" fill="none" strokeLinecap="round" />
            ) : isWow ? (
              <ellipse cx="0" cy="26" rx="8" ry="10" fill="#1A0936" />
            ) : (
              <path d="M -14 22 Q 0 36 14 22" stroke="#1A0936" strokeWidth="3" fill="none" strokeLinecap="round" />
            )}
            {/* cheeks */}
        ...
        {mood === "excited" && (
          <>
            <motion.text x="20" y="40" fontSize="22"
              animate={{ opacity: [0, 1, 0], y: [40, 20, 0] }}
              transition={{ duration: 1.4, repeat: Infinity }}>✨</motion.text>
            <motion.text x="160" y="50" fontSize="22"
              animate={{ opacity: [0, 1, 0], y: [50, 30, 10] }}
              transition={{ duration: 1.4, repeat: Infinity, delay: 0.3 }}>⭐</motion.text>
          </>
        )}
      </svg>
    </motion.div>
