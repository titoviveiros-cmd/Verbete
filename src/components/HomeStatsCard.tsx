...
type Stats = {
  games_played: number; games_won: number; current_streak: number; best_streak: number;
...
export function HomeStatsCard() {
  ...
  useEffect(() => {
    ...
    const hour = (() => { const d = new Date(); d.setMinutes(0, 0, 0); return d.toISOString(); })();
    (async () => {
      const [{ data: s }, { data: a }] = await Promise.all([
        supabase.from("user_stats")
          .select("games_played, games_won, current_streak, best_streak, last_played_date")
          .eq("user_id", user.id).maybeSingle(),
        supabase.from("daily_attempts").select("id")
          .eq("user_id", user.id).eq("challenge_hour", hour).maybeSingle(),
      ]);
      setStats(s as Stats);
      setPlayedThisHour(!!a);
    })();
          ...
          >
            {streak > 0 && (
              <span
                ...
                style={{ background: "radial-gradient(circle, rgba(255,140,40,0.7) 0%, transparent 70%)" }}
              />
            )}
            <div className="relative text-2xl leading-none">🔥</div>
            <div className="relative font-display text-xl leading-tight text-sun">{streak}</div>
            <div className="relative text-[9px] uppercase tracking-wider text-muted-foreground">streak</div>
          </motion.div>
          <div className="flex-1 grid grid-cols-3 gap-1 text-center min-w-0">
            <div className="min-w-0">
              <div className="font-display text-lg leading-none text-primary">{games}</div>
              <div className="text-[9px] uppercase tracking-wider text-muted-foreground truncate">jogos</div>
            </div>
            <div className="min-w-0">
              <div className="font-display text-lg leading-none text-mint">{wins}</div>
              <div className="text-[9px] uppercase tracking-wider text-muted-foreground truncate">vitórias</div>
            </div>
            <div className="min-w-0">
              <div className="font-display text-lg leading-none text-pink">{best}</div>
              <div className="text-[9px] uppercase tracking-wider text-muted-foreground truncate">recorde</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {atRisk && (
        ...
        >
          ...
          <div className="flex-1 text-xs leading-tight">
            <p className="font-display text-sm text-foreground">Sua streak está em risco!</p>
            <p className="text-muted-foreground">Jogue uma rodada hoje para manter os {streak} 🔥</p>
          </div>
          <Link
            to="/daily"
